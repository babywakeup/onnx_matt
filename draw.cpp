...
{
    //img  预处理前的图像，matte是模型的输出， bg是我们想要的背景
    cv::resize(matte, matte, img.size()); // 将matte的大小变成了img的大小，参数分别是：输入图像，输出图像，输出图像的大小
    if (format == "foreground")
    {
        // 返回前景部分
        float gray_value = matte.at<float>(0, 0);
        cv::cvtColor(matte, matte, cv::COLOR_GRAY2BGR); // 将matte_temp的通道数变成了3，0-1之间大小


        img.convertTo(img, CV_32FC3); // 将img的数据类型转换成CV_32FC3，0-255之间
    ...
    else if (format == "matte")
    {
        ...
        // 乘以 255
        cv::multiply(matte, cv::Scalar(255.0, 255.0, 255.0), matte); // 将matte和(255, 255, 255)对应位置的元素相乘，结果保存在matte中
        // 转为 8UC3
